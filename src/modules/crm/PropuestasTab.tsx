...
export function PropuestasTab() {
  ...
  const [header, setHeader] = useState({
    sku: "", id_tipo_propuesta: "", id_cliente: "", id_vendedor: "",
  ...
  const loadPropuestas = async () => {
    ...
    let q = supabase
      .from("propuestas")
      .select("id, sku, fecha, total_general, total_voucher, total_en_bolsas_soja, total_en_bolsas_maiz, total_en_bolsas_trigo, created_at, id_cliente, id_vendedor, id_tipo_propuesta, id_estado_propuesta, tipo_propuesta(codigo), estado_propuesta(codigo), clientes(nombre)")
    ...
    if (!error && data && data.length > 0) {
      ...
      setRows((data as any[]).map((d) => ({
        id: d.id, sku: d.sku, fecha: d.fecha,
        total_general: d.total_general, total_voucher: d.total_voucher,
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [c, p, d, t, v] = await Promise.all([
        ...
        supabase.from("productos").select("id, nombre, fabricante, precio_final, contenido_empaque, voucher, precio_minimo, categorias_producto(descripcion), unidades_medida(descripcion)").eq("estado", "activo"),
      ...
      // Load latest CBOT prices for bolsa calculation
      ...
      for (const cult of culturas) {
        const { data: cbotData } = await supabase
          .from("cbot")
          .select("precio_bolsa, culturas(codigo)")
          .order("created_at", { ascending: false })
          .limit(1);
        if (cbotData && cbotData.length > 0) {
          // Try to get price for this specific culture
          const { data: cbotCult } = await supabase
            .from("cbot")
            .select("precio_bolsa, culturas!inner(codigo)")
            .eq("culturas.codigo", cult)
            .order("created_at", { ascending: false })
            .limit(1);
          if (cbotCult && cbotCult.length > 0) {
            const key = cult.toLowerCase() as "soja" | "maiz" | "trigo";
            prices[key] = (cbotCult[0] as any).precio_bolsa ?? 0;
          }
        }
      }
  ...
  const resetForm = () => {
    setHeader({ sku: "", id_tipo_propuesta: "", id_cliente: "", id_vendedor: perfil?.id ?? "", fecha: new Date().toISOString().slice(0, 10) });
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!header.id_cliente || !header.id_tipo_propuesta || cart.length === 0) {
      alert("Complete cliente, tipo y al menos un producto."); return;
    }
    setSaving(true);
    const tipoVenta = tipos.find((t) => t.codigo === "venta")?.id;
    const estadoVigente = await supabase.from("estado_propuesta").select("id").eq("codigo", "vigente").single().then((r) => r.data?.id);
    const { data: prop } = await supabase.from("propuestas").insert({
      sku: header.sku || null, id_cliente: header.id_cliente,
      id_tipo_propuesta: header.id_tipo_propuesta, fecha: header.fecha,
      id_vendedor: header.id_vendedor || perfil?.id,
      total_items: cart.length,
      total_voucher: header.id_tipo_propuesta === tipoVenta ? totalVoucher : 0,
      total_en_bolsas: 0,
      total_en_bolsas_soja: totalEnBolsas.soja,
      total_en_bolsas_maiz: totalEnBolsas.maiz,
      total_en_bolsas_trigo: totalEnBolsas.trigo,
      total_general: totalGeneral, id_estado_propuesta: estadoVigente,
    }).select("id").single();

    if (!prop?.id) { setSaving(false); return; }

    for (const it of cart) {
      await supabase.from("productos_propuesta").insert({
        id_propuesta: prop.id, id_producto: it.id_producto,
        id_distribuidor: it.id_distribuidor || null,
        categoria: it.categoria, unidad_medida: it.unidad_medida,
        contenido_empaque: it.contenido_empaque, voucher: it.voucher_pct,
        precio_minimo: it.precio_minimo, precio_producto: it.precio_producto,
        cantidad: it.cantidad, num_aplicaciones: it.num_aplicaciones,
        dosis_ha: it.dosis_ha, area_tratada: it.area_tratada,
        costo_ha: it.costo_ha, importe_total: it.importe_total,
      });
    }
    ...
    if (header.id_tipo_propuesta === tipoVenta && totalVoucher > 0) {
      ...
      if (vouch) {
        ...
        await supabase.from("vouchers").insert({ id_cliente: header.id_cliente, valor_total_generado: totalVoucher, valor_total_liberado: 0, valor_restante: totalVoucher });
      }
      const { data: v2 } = await supabase.from("vouchers").select("id").eq("id_cliente", header.id_cliente).single();
      if (v2) {
        await supabase.from("movimiento_vouchers").insert({ id_voucher: (v2 as any).id, id_cliente: header.id_cliente, id_propuesta: prop.id, valor_generado: totalVoucher, tipo: "generado", id_usuario: perfil?.id });
      }
    }
  ...
  const handleCancelar = async (row: PropuestaRow) => {
    ...
    await supabase.from("propuestas").update({ id_estado_propuesta: estadoCancelado }).eq("id", row.id);
    const { data: v } = await supabase.from("vouchers").select("id, valor_restante").eq("id_cliente", (prop as any).id_cliente).single();
    if (v) {
      const rest = Number(((v as any).valor_restante - ((prop as any).total_voucher || 0)).toFixed(2));
      await supabase.from("vouchers").update({ valor_restante: Math.max(0, rest) }).eq("id", (v as any).id);
      await supabase.from("movimiento_vouchers").insert({ id_voucher: (v as any).id, id_cliente: (prop as any).id_cliente, id_propuesta: row.id, valor_liberado: (prop as any).total_voucher, tipo: "cancelado", id_usuario: perfil?.id });
    }
    await loadPropuestas();
  };
              ...
              <tr key={r.id} className={`hover:bg-gray-50 transition-colors ${r.estado_codigo === "cancelado" ? "opacity-60" : ""}`}>
                ...
                <td className="px-4 py-3 text-right">
                  <div className="flex items-center justify-end gap-2">
                    ...
                    <button type="button" className={btnGhost} onClick={() => generarPdfPropuesta(supabase, r.id, false)}>
                      <i className="fas fa-file-pdf" />PDF
                    </button>
                    {isAdmin && (
                      <button type="button" className={btnGhost} onClick={() => generarPdfPropuesta(supabase, r.id, true)}>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[95vh] overflow-y-auto">
            ...
            <div className="flex justify-between gap-3 px-6 py-4 border-t border-gray-100">
              <div className="flex gap-2">
                <button type="button" className={btnGhost} onClick={() => generarPdfPropuesta(supabase, viewing.id, false)}>
                  <i className="fas fa-file-pdf" />PDF
                </button>
                {isAdmin && (
                  <button type="button" className={btnGhost} onClick={() => generarPdfPropuesta(supabase, viewing.id, true)}>
                    <i className="fas fa-file-pdf text-emerald-600" />PDF margen
                  </button>
                )}
              </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                    <div>
                      ...
                      <input className={inputCls} value={header.sku} onChange={(e) => setHeader({ ...header, sku: e.target.value })} placeholder="Opcional" />
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <select className={inputCls} value={header.id_cliente} onChange={(e) => setHeader({ ...header, id_cliente: e.target.value })} required>
                        <option value="">Seleccione</option>
                    ...
                    <div>
                      ...
                      <select className={inputCls} value={header.id_vendedor} onChange={(e) => setHeader({ ...header, id_vendedor: e.target.value })}>
                        <option value="">Seleccione</option>
                    ...
                    <div>
                      ...
                      <input type="date" className={inputCls} value={header.fecha} onChange={(e) => setHeader({ ...header, fecha: e.target.value })} max={new Date().toISOString().slice(0, 10)} />
                    </div>
                  </div>
                </div>
                  ...
                  {selectedProduct && (
                    <div className="space-y-3">
                      ...
                      <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 flex flex-wrap gap-4 text-xs">
                        ...
                        <div><span className="text-blue-500 font-bold">Un. Medida:</span> <span className="text-blue-800">{selectedProduct.unidades_medida?.descripcion ?? "—"}</span></div>
