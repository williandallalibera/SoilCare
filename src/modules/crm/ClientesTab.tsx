...
export function ClientesTab() {
  ...
  const loadClientes = async () => {
    ...
    if (isReviewMode) {
      ...
      setRows([
        { id: "cl-1", nombre: "Fazenda Santa Maria", email: "contato@santamaria.com", estado: "activo", created_at: new Date().toISOString(), vendedor_nombre: "Admin Sistema" },
        { id: "cl-2", nombre: "Agroindustrial Los Abuelos", email: "info@losabuelos.com.py", estado: "activo", created_at: new Date().toISOString(), vendedor_nombre: "RTV Carlos" },
        { id: "cl-3", nombre: "Cooperativa Multiactiva Ltda", email: "administracao@coop.com", estado: "activo", created_at: new Date().toISOString(), vendedor_nombre: "Admin Sistema" },
        { id: "cl-4", nombre: "Estancia El Rodeo", email: "rodeo@ranch.com", estado: "inactivo", created_at: new Date().toISOString(), vendedor_nombre: "RTV Carlos" }
      ] as any);
      return;
    }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (editing) {
      ...
      const { data: authData, error: authError } = await supabase.auth.signUp({
        email,
        password,
        options: { data: { nombre: form.nombre.trim(), perfil_acceso: "cliente" } },
      });
      if (authError) {
        alert("No se pudo crear el usuario. " + (authError.message || "Verifique el email."));
        setSaving(false);
        return;
      }
      ...
      if (userId) {
        await supabase.from("usuarios").insert({
          id: userId, nombre: form.nombre.trim(), email, perfil_acceso: "cliente", estado: "activo",
        });
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-50">
            {filteredRows.map((r) => (
              <tr
                key={r.id}
                className={`hover:bg-gray-50 transition-colors ${r.estado === "inactivo" ? "opacity-60" : ""}`}
              >
              ...
              <div className="p-6 space-y-5">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {!editing && (
                      <div>
                        <label className={labelCls}>Contraseña inicial (opcional)</label>
                        <input type="password" className={inputCls} placeholder="Se generará automáticamente" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} />
                      </div>
                    )}
                    <div>
                      <label className={labelCls}>Estado</label>
