...
export function RteTab() {
    ...
    const [modalRte, setModalRte] = useState<{ rteId: string; monitoreo: { id: string; id_cliente: string; id_parcela: string; id_zafra: string; hectares: number | null; cliente_nombre: string; parcela_nombre: string; zafra_nombre: string } } | null>(null);

    const loadRows = async () => {
        setLoading(true);
        const isReviewMode = localStorage.getItem("forceAuthReview") === "true";
        if (isReviewMode) {
            console.log("RteTab: Review Mode - Injecting mock rte");
            setRows([
                { id: "r-1", id_monitoreo: "m-1", id_cliente: "cl-1", costo_total: 1000, ingreso_total: 5000, resultado_tecnico: 4000, cliente_nombre: "Fazenda Santa Maria", parcela_nombre: "Lote 1", zafra_nombre: "2023/2024" }
            ]);
    ...
    const handlePdf = async (id: string) => {
        setGettingPdf(id);
        await generarPdfRte(supabase, id);
        setGettingPdf(null);
    };
                            ...
                            <tr key={r.id} className="hover:bg-gray-50 transition-colors">
                                ...
                                <td className="px-4 py-3 text-right space-x-2">
                                    <button
                                        ...
                                        onClick={() => setModalRte({ rteId: r.id, monitoreo: { id: r.id_monitoreo, id_cliente: r.id_cliente, id_parcela: "", id_zafra: "", hectares: null, cliente_nombre: r.cliente_nombre, parcela_nombre: r.parcela_nombre, zafra_nombre: r.zafra_nombre } })}
                                    >
                                        Editar
                                    </button>
                                    <button
