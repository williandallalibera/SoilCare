...
export function EvaluacionesTab() {
  ...
  const loadRows = async () => {
    ...
    if (isReviewMode) {
      console.log("EvaluacionesTab: Review Mode - Injecting mock evaluaciones");
      setRows([
        { id: "e-1", id_cliente: "cl-1", fecha_evaluacion: new Date().toISOString(), fecha_proxima_evaluacion: new Date(Date.now() + 86400000 * 7).toISOString(), cliente_nombre: "Fazenda Santa Maria", parcela_nombre: "Lote 1", zafra_nombre: "2023/2024" }
      ]);
  ...
  const handlePdf = async (id: string) => {
    setGettingPdf(id);
    await generarPdfEvaluacion(supabase, id);
    setGettingPdf(null);
  };
              ...
              <tr key={r.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3 text-right space-x-2">
                  <button
                    ...
                    onClick={() => setModalEvaluacionId(r.id)}
                  >
                    Editar
                  </button>
                  <button
