...
export function SiembraTab() {
  ...
  const loadRows = async () => {
    ...
    if (isReviewMode) {
      console.log("SiembraTab: Review Mode - Injecting mock siembras");
      setRows([
        { id: "s-1", id_monitoreo: "m-1", fecha_inicio: new Date(Date.now() - 86400000 * 30).toISOString().slice(0, 10), fecha_termino: new Date(Date.now() - 86400000 * 25).toISOString().slice(0, 10), costo_total: 12000, costo_ha: 120, id_cliente: "cl-1", id_parcela: "p-1", id_zafra: "z-1", hectares: 100, cliente_nombre: "Fazenda Santa Maria", parcela_nombre: "Lote 1", zafra_nombre: "2023/2024" }
      ]);
      setClientes([{ id: "cl-1", nombre: "Fazenda Santa Maria" }]);
      setParcelas([{ id: "p-1", nombre_parcela: "Lote 1" }]);
  ...
  const handlePdf = async (id: string) => {
    setGettingPdf(id);
    await generarPdfSiembra(supabase, id);
    setGettingPdf(null);
  };
              ...
              <tr key={r.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3 text-right space-x-2">
                  <button
                    ...
                    onClick={() => setModalSiembra({ siembraId: r.id, monitoreo: { id: r.id_monitoreo, id_cliente: r.id_cliente, id_parcela: r.id_parcela, id_zafra: r.id_zafra, hectares: r.hectares, cliente_nombre: r.cliente_nombre, parcela_nombre: r.parcela_nombre, zafra_nombre: r.zafra_nombre }, areaHa: r.hectares })}
                  >
                    Editar
                  </button>
                  <button
